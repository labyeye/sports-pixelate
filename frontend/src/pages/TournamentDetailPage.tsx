...
function FixtureCard({ fixture, onSave }: { fixture: Fixture; onSave: (id: string, scoreA: number, scoreB: number) => Promise<void> }) {
...
export default function TournamentDetailPage() {
  ...
  const generateFixtures = async (regenerate: boolean) => {
    ...
    setGenerating(true);
    try {
      const r = await tournamentAPI.generateFixtures(id, { regenerate, shuffle: true });
      setFixtures(r.data);
    ...
    <AppLayout title={tournament.name}>
      ...
      <div className="border-2 border-black bg-white p-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {tournament.teams.length === 0 && <p className="text-xs text-muted-foreground">No teams added yet</p>}
        </div>
        {!tournament.fixturesGenerated && (
