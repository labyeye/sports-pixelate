  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      (attRes.data || []).forEach((r: any) => {
        if (r.date?.slice(0, 10) === date) existing[r.student._id] = r.status;
      });
      setMarks(existing);
    } catch (e: any) {
    ...
    <AppLayout title="Student Attendance">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <button
          onClick={saveAll}
          disabled={saving}
          className="border-2 bg-[#024BAB] text-white px-4 py-2 text-sm flex items-center gap-1.5 disabled:opacity-60"
        >
          {saving ? (
