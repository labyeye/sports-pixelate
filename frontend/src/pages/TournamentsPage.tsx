...
import { Trophy, Plus, Check, X, Loader2, Users, ChevronRight } from "lucide-react";
...
export default function TournamentsPage() {
  ...
  const resetForm = () => {
    setForm({ name: "", sport: "", format: "knockout", startDate: "", endDate: "", venue: "" });
    setShowForm(false);
  };
  ...
  const handleCreate = async () => {
    ...
    if (!form.name.trim() || !form.sport.trim()) {
      toast({ title: "Missing fields", description: "Name and sport are required", variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <AppLayout title="Tournaments">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h1 className="font-display font-bold text-2xl text-black">Tournaments</h1>
        <button
          onClick={() => {
            resetForm();
            setShowForm(true);
      ...
      {showForm && (
        <div className="bg-white border-2 border-black p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase mb-1">Tournament Name *</label>
              <input
                value={form.name}
            ...
            <div>
              ...
              <select
                value={form.format}
                onChange={(e) => setForm((p) => ({ ...p, format: e.target.value as "knockout" | "round_robin" }))}
          ...
          <p className="text-xs text-muted-foreground mt-3">
            Add teams and generate fixtures from the tournament page after creating it.
          </p>
      ...
      {loading ? (
        ...
        <div className="border-2 border-black bg-white p-12 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Create one to start building fixtures</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span className={cn("text-[10px] font-bold uppercase px-2 py-1 border-2 border-black", meta.bg, meta.text)}>
                    {t.status.replace("_", " ")}
                  </span>
                </div>
                <p className="font-bold text-black">{t.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{t.sport} · {t.format === "knockout" ? "Knockout" : "Round Robin"}</p>
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-black/10">
                  <span className="text-xs font-bold text-black flex items-center gap-1">
                    <Users className="w-3.5 h-3.5" /> {t.teams.length} team{t.teams.length === 1 ? "" : "s"}
                  </span>
