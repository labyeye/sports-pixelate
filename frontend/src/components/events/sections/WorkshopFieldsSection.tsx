import { GraduationCap, UserCog, Armchair, Clock, Package } from "lucide-react";
  ...
  return (
    <CollapsibleSection title="Workshop Details" icon={GraduationCap}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className={labelClass}>
            <Armchair className="w-3.5 h-3.5 text-[#024BAB]" />
            Max Seats
          </label>
          <input
            type="number"
        ...
        <label className="flex items-center gap-2 text-sm font-bold">
          <input
            ...
            onChange={(e) =>
              onChange({ certificateAvailable: e.target.checked })
            }
          />
