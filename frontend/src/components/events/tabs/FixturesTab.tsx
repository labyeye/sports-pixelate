...
export function FixturesTab({ event, onChanged }: Props) {
  ...
  const generateFixtures = async (regenerate: boolean) => {
    ...
    setGenerating(true);
    try {
      const r: any = await eventAPI.generateFixtures(event._id, {
        regenerate,
        shuffle: true,
      });
      setFixtures(r.data || []);
  ...
  const formatSupported =
    event.format === "knockout" || event.format === "round_robin";
    ...
    <div className="space-y-4">
      {!formatSupported && (
        <div className="border-2 border-black bg-white p-4 text-xs text-muted-foreground">
          Fixture generation is coming soon for the "{event.format}" format.
          Only Knockout and Round Robin support automatic bracket generation
          today.
        </div>
      )}
          ...
          {!event.fixturesGenerated && roundsPreview.length > 0 && (
            <div className="mb-4">
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">
                Structure Preview ({roundsPreview.length} round
                {roundsPreview.length === 1 ? "" : "s"})
              </h4>
              <div className="flex gap-3 overflow-x-auto pb-1">
                {roundsPreview.map((r) => (
                  <div
                    key={r.round}
                    className="border-2 border-black/20 px-3 py-2 shrink-0 min-w-32"
                  >
                    <p className="text-xs font-bold text-black">{r.label}</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      {r.matchCount} match{r.matchCount === 1 ? "" : "es"}
                      {r.byes > 0
                        ? ` · ${r.byes} bye${r.byes === 1 ? "" : "s"}`
                        : ""}
                    </p>
                  </div>
                ))}
        ...
        formatSupported && (
          <div className="border-2 border-black bg-white p-12 flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Generate fixtures once teams are added.
            </p>
          </div>
        )
      )}
