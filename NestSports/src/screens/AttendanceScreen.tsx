...
import { requestSelfMarkPermissions, getCurrentPosition } from '../utils/location';
import {
  Clock,
  Search,
  CheckCircle2,
...
import { attendanceAPI, employeeAPI, attendanceCorrectionAPI, RNFile } from '../api/client';
...
export default function AttendanceScreen() {
  ...
  const hasFaceEnrolled =
    Array.isArray(myEmployee?.faceDescriptor) && myEmployee.faceDescriptor.length === 128;

  const todayStr = toDateStr(new Date());
  const todayRecord = isEmployee && dateFilter === todayStr ? records[0] : undefined;
        ...
        async result => {
          ...
          try {
            ...
            await attendanceAPI.selfMark(
              { action, lat: pos.latitude, lng: pos.longitude, accuracy: pos.accuracy },
              selfie,
            );
  ...
  const markAbsent = (empId: string) => {
    Alert.alert('Mark Absent', `Mark this employee as absent for ${dateFilter}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await attendanceAPI.mark({ employee: empId, date: dateFilter, status: 'absent' });
            await load();
          } catch (e: any) {
            Alert.alert('Error', e.message);
          }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {isEmployee && todayRecord?.approvalPending && (
        <View style={styles.approvalBanner}>
          ...
          <Text style={styles.approvalBannerText}>
            Today's attendance is pending HR approval (late check-in exceeded your monthly
            allowance)
          </Text>
        </View>
      )}
      ...
      {isEmployee && myEmployee?.geofenceAttendanceEnabled && dateFilter === todayStr && (
        <View style={styles.selfMarkCard}>
          <View style={styles.selfMarkHeader}>
            ...
            <Text style={styles.selfMarkHeaderText}>Geofenced Mobile Check-in</Text>
          </View>
          {!hasFaceEnrolled ? (
            <>
              <Text style={styles.selfMarkNoteText}>
                Enroll your face first using the camera — this is required before you can check
                in from the app.
              </Text>
              <TouchableOpacity
            ...
            <View style={styles.selfMarkDoneRow}>
              ...
              <Text style={styles.selfMarkDoneText}>Checked in and out for today</Text>
            </View>
          )}
        </View>
      )}
          ...
          {Object.entries(summary).map(([status, count]) => {
            ...
            return (
              ...
              >
                ...
                <Text
                  style={[styles.summaryStatus, { color: active ? colors.white : cfg.color }]}
                >
                  {status.replace('_', ' ')}
                </Text>
              </TouchableOpacity>
              ...
              <View style={[styles.card, { borderColor: colors.black }]}>
                <View style={styles.cardRow}>
                  <View style={styles.photoWrap}>
                    {emp?.avatar ? (
                      <Image source={{ uri: emp.avatar }} style={styles.empPhoto} />
                    ) : (
                      <View style={[styles.empPhoto, styles.empPhotoFallback, { backgroundColor: cfg.bg }]}>
                        <Text style={[styles.empPhotoInitials, { color: cfg.color }]}>{initials}</Text>
                      </View>
                    )}
                    <View
                      style={[
                  ...
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={styles.empName} numberOfLines={1}>
                      {emp ? `${emp.firstName} ${emp.lastName}`.toUpperCase() : 'UNKNOWN'}
                    </Text>
                ...
                {(workHours || overtime > 0 || item.earlyLeaving || item.checkInLocation || item.checkOutLocation) && (
                  <View style={styles.timeRow}>
                    {workHours ? (
                      <View style={styles.timePill}>
                        <Clock size={11} color={colors.blue} strokeWidth={2.5} />
                        <Text style={styles.timeText}>{workHours}h</Text>
                      </View>
                    ) : null}
                    {overtime > 0 && (
                      <View style={[styles.timePill, { borderColor: colors.yellow }]}>
                        <Clock size={11} color={colors.yellow} strokeWidth={2.5} />
                        <Text style={[styles.timeText, { color: colors.yellow }]}>OT: {overtime}h</Text>
                      </View>
                    )}
                    {item.earlyLeaving && (
                      <View style={[styles.timePill, { borderColor: colors.blue }]}>
                        <LogOut size={11} color={colors.blue} strokeWidth={2.5} />
                        <Text style={[styles.timeText, { color: colors.blue }]}>Early Leave</Text>
                      </View>
                    )}
                    {(item.checkInLocation || item.checkOutLocation) && (
                      <TouchableOpacity style={styles.timePill} onPress={() => showLocationOptions(item)}>
                        <MapPin size={11} color={colors.blue} strokeWidth={2.5} />
          ...
          <SafeAreaView style={styles.safe} edges={['top']}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  const eligible = employees.filter(e => !isWeekendForEmployee(dateFilter, e));
              ...
              >
                <View style={styles.bulkCheckbox}>
                  {bulkSelected.length === employees.filter(e => !isWeekendForEmployee(dateFilter, e)).length &&
              ...
              {employees.map(emp => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => setBulkSelected(p => (checked ? p.filter(id => id !== emp._id) : [...p, emp._id]))}
                    activeOpacity={0.7}
                  >
                ...
                onPress={async () => {
                  ...
                  try {
                    await attendanceAPI.bulkMark({
                      records: bulkSelected.map(id => ({ employee: id, date: dateFilter, status: bulkStatus })),
                    });
                ...
                <View>
                  ...
                  <ScrollView style={{ maxHeight: 180 }} nestedScrollEnabled>
                    {employees.map(e => (
                      <TouchableOpacity
                        key={e._id}
                        style={[styles.empOption, selectedEmpId === e._id && styles.empOptionActive]}
                        onPress={() => setSelectedEmpId(e._id)}
                      >
                        <Text style={[styles.empOptionName, selectedEmpId === e._id && { color: colors.white }]}>
                          {e.firstName} {e.lastName}
                        </Text>
                        <Text style={[styles.empOptionId, selectedEmpId === e._id && { color: '#93C5FD' }]}>{e.employeeId}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                </View>
      ...
      {isEmployee && (
        <Modal visible={showCorrectionModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowCorrectionModal(false)}>
          <SafeAreaView style={styles.safe} edges={['top']}>
            <View style={styles.modalHeader}>
            ...
            <ScrollView contentContainerStyle={{ padding: 20, gap: 16 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <DateTimeField label="Date *" mode="date" value={correctionForm.date} onChangeText={v => setCorrectionForm(p => ({ ...p, date: v }))} />
              ...
              <View>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 6 }}>
                  {['regularization', 'missed_punch'].map(t => (
                    ...
                    >
                      <Text style={[styles.selChipText, correctionForm.type === t && { color: colors.white }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <View style={{ flex: 1 }}>
                  <DateTimeField
                    ...
                    onChangeText={v => setCorrectionForm(p => ({ ...p, checkIn: v }))}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <DateTimeField
                    ...
                    onChangeText={v => setCorrectionForm(p => ({ ...p, checkOut: v }))}
                  />
                </View>
              </View>
              ...
              <TouchableOpacity style={styles.submitBtn} onPress={handleCreateCorrection} disabled={submittingCorrection}>
...
const styles = StyleSheet.create({
  ...
  approvalBannerText: { flex: 1, fontFamily: FONT.medium, fontSize: 12, fontWeight: '600', color: '#92400E' },
  selfMarkCard: { backgroundColor: colors.white, borderBottomWidth: 2, borderBottomColor: colors.black, padding: 14, gap: 10 },
  selfMarkHeader: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  selfMarkHeaderText: { fontFamily: FONT.bold, fontSize: 11, fontWeight: '700', color: colors.blue, textTransform: 'uppercase', letterSpacing: 0.4 },
  selfMarkBtn: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  selfMarkBtnText: { color: colors.white, fontFamily: FONT.bold, fontSize: 13, fontWeight: '700', textTransform: 'uppercase' },
  selfMarkDoneRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  selfMarkDoneText: { fontFamily: FONT.bold, fontSize: 13, fontWeight: '700', color: colors.green },
  selfMarkNoteText: { fontFamily: FONT.medium, fontSize: 12, color: colors.muted, fontWeight: '500', marginBottom: 4 },
  searchWrap: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  empPhoto: { width: 48, height: 48, borderRadius: 24, borderWidth: 2, borderColor: colors.black },
  ...
  empDeptRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 3 },
  empDeptText: { fontFamily: FONT.medium, fontSize: 11, color: '#9CA3AF', fontWeight: '500' },
  ...
  empName: { fontFamily: FONT.bold, fontSize: 15, fontWeight: '800', color: colors.black, letterSpacing: 0.2 },
  empSub: { fontFamily: FONT.medium, fontSize: 11, color: '#9CA3AF', fontWeight: '600', marginTop: 2 },
  ...
  timeText: { fontFamily: FONT.bold, fontSize: 12, fontWeight: '700', color: colors.black },
  ...
  empOptionName: { fontFamily: FONT.bold, fontSize: 13, fontWeight: '700', color: colors.black },
  ...
  bulkEmpRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
