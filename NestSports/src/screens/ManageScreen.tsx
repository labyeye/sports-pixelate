    ...
    <SafeAreaView edges={['top']} style={styles.screen}>
      ...
      >
        ...
        <Card>
          ...
          <Row
            title="Departments"
            subtitle="Manage teams and departments"
            onPress={() => navigation?.navigate?.('Departments')}
          />
          <Row
            title="Employees"
            subtitle="Manage staff and coaches"
            onPress={() => navigation?.navigate?.('Employees')}
          />
          <Row
            title="Facilities"
            subtitle="Sports facilities and rates"
            onPress={() => navigation?.navigate?.('Facilities')}
          />
          <Row
            title="Coaching Plans"
            subtitle="Sports plans and pricing"
          ...
          {designations.length > 0 ? (
            designations.map(d => (
              <Row
                key={d._id}
                title={d.name}
                subtitle={`${d.department?.name || '—'} · Grade ${
                  d.grade || '—'
                }`}
              />
            ))
