    ...
    <SafeAreaView edges={['top']} style={styles.screen}>
      ...
      {batch ? (
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No students in this batch" icon={Layers} />}
          renderItem={({ item }) => (
            <Card>
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}
          showsVerticalScrollIndicator={false}
          ListEmptyComponent={<EmptyState title="No batches found" icon={Layers} />}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => setBatch(item.batch)} activeOpacity={0.85}>
              <Card>
                ...
                <View style={styles.statsRow}>
                  ...
                  <View style={styles.statCol}>
                    ...
                    <Text style={[styles.statValue, { color: colors.muted }]}>{item.inactive}</Text>
                  </View>
                  <View style={styles.statCol}>
                    <Text style={styles.statLabel}>On Hold</Text>
                    <Text style={[styles.statValue, { color: colors.orange }]}>{item.on_hold}</Text>
                  </View>
                </View>
              </Card>
            </TouchableOpacity>
