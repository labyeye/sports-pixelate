...
import { Clock, CheckCircle2, XCircle, AlarmClock, CalendarDays, Clock3 } from "lucide-react-native";
...
export default function MyReportScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const holidays = records.filter((r: any) => r.status === "holiday" || r.status === "weekend").length;
    ...
    <SafeAreaView edges={["top"]} style={styles.screen}>
      ...
      >
        ...
        <View style={styles.kpiGrid}>
          <KpiTile label="Attendance Rate" value={`${rate}%`} sub={`${summary.present}/${summary.total} days`} color={colors.blue} icon={Clock} />
          <KpiTile label="Present" value={summary.present} color={colors.green} icon={CheckCircle2} />
          <KpiTile label="Absent" value={summary.absent} color={colors.red} icon={XCircle} />
          <KpiTile label="Late" value={summary.late} color={colors.orange} icon={AlarmClock} />
          <KpiTile label="On Leave" value={summary.onLeave} color={colors.purple} icon={CalendarDays} />
          <KpiTile label="Half Day" value={summary.halfDay} color={colors.yellow} icon={Clock3} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
