  ...
  return (
    <SafeAreaView edges={["top"]} style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.subtitle}>Attendance deduction rules (read-only)</Text>

        <Card>
          <SectionTitle title="Shift Timing" />
          <Row title="Shift Hours" subtitle={`${fmt(rules.shiftStartHour, rules.shiftStartMinute)} – ${fmt(rules.shiftEndHour, rules.shiftEndMinute)}`} />
        </Card>

        <Card>
          <SectionTitle title="Late Arrival" />
          <Row title="Grace Period" subtitle={`${rules.lateThresholdMinutes ?? 0} minutes`} />
          <Row
            title="Deduction"
            subtitle={
        ...
        <Card>
          ...
          <Row title="Threshold" subtitle={`Late by more than ${rules.halfDayThresholdMinutes ?? 0} min = half day (50% pay)`} />
        </Card>
        ...
        <Card>
          ...
          {rules.earlyCheckoutDeductionEnabled && (
            <Row title="Grace Period" subtitle={`${rules.earlyCheckoutThresholdMinutes ?? 0} minutes before shift end`} />
          )}
        </Card>
      </ScrollView>
    </SafeAreaView>
