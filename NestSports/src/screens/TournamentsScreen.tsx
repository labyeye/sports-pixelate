...
export default function TournamentsScreen({ navigation }: any) {
  ...
  const load = useCallback(
    () => tournamentAPI.getAll().then((res: any) => res.success && setTournaments(res.data || [])),
    [],
  );
    ...
    <SafeAreaView edges={["top"]} style={styles.screen}>
      ...
      >
        {tournaments.length === 0 ? (
          <Card>
            <EmptyState title="No tournaments yet" sub="Create one to start building fixtures" icon={Trophy} />
          </Card>
        ) : (
          tournaments.map((t: any) => (
            <TouchableOpacity key={t._id} onPress={() => navigation.navigate("TournamentDetail", { id: t._id })}>
              <Card>
                <View style={styles.topRow}>
                  <View style={styles.iconChip}>
                    <Trophy size={16} color={colors.blue} strokeWidth={2.5} />
                  </View>
                  <Badge label={t.status?.replace(/_/g, " ") || "-"} color={STATUS_COLORS[t.status] || colors.blue} />
                </View>
                <Text style={styles.name}>{t.name}</Text>
                <Text style={styles.meta}>
                  {t.sport} · {t.format === "knockout" ? "Knockout" : "Round Robin"}
                </Text>
                <View style={styles.bottomRow}>
                  <View style={styles.teamsCount}>
                    <Users size={13} color={colors.black} strokeWidth={2.5} />
                    <Text style={styles.teamsCountText}>
                      {t.teams?.length || 0} team{t.teams?.length === 1 ? "" : "s"}
                    </Text>
                  </View>
