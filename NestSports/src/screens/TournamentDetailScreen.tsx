...
import { ScrollView, View, Text, TextInput, TouchableOpacity, RefreshControl, StyleSheet, Alert, ActivityIndicator } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Plus, Trash2, Shuffle, RotateCcw, Check, X, Trash } from "lucide-react-native";
...
export default function TournamentDetailScreen({ route, navigation }: any) {
  ...
  const generateFixtures = (regenerate: boolean) => {
    const run = async () => {
      setGenerating(true);
      try {
        const res: any = await tournamentAPI.generateFixtures(id, { regenerate, shuffle: true });
  ...
  const deleteTournament = () => {
    Alert.alert("Delete tournament?", "This removes the tournament and all its fixtures.", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
    ...
    <SafeAreaView edges={["top"]} style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          {tournament.sport} · {tournament.format === "knockout" ? "Knockout" : "Round Robin"}
        ...
        <Card style={{ marginTop: 14 }}>
          ...
          <View style={styles.teamChips}>
            ...
            {tournament.teams.length === 0 && <Text style={styles.emptyHint}>No teams added yet</Text>}
          </View>
          ...
          {!tournament.fixturesGenerated && (
            <View style={{ flexDirection: "row", gap: 8, marginTop: 4 }}>
              ...
              />
              <TouchableOpacity onPress={addTeam} disabled={addingTeam || !newTeam.trim()} style={styles.addTeamBtn}>
                {addingTeam ? <ActivityIndicator color={colors.blue} size="small" /> : <Plus size={16} color={colors.blue} strokeWidth={2.5} />}
              </TouchableOpacity>
            </View>
          )}
          ...
          <View style={styles.genRow}>
            {!tournament.fixturesGenerated ? (
              <TouchableOpacity
                ...
                style={[styles.genBtn, (tournament.teams.length < 2 || generating) && { opacity: 0.5 }]}
              >
          ...
          {tournament.teams.length < 2 && (
            <Text style={styles.emptyHint}>Add at least 2 teams to generate fixtures.</Text>
          )}
        </Card>
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: FONT.bold, fontWeight: "800", fontSize: 22, color: colors.black },
  subtitle: { fontFamily: FONT.medium, fontSize: 13, color: colors.muted, marginTop: 4 },
  ...
  genRow: { marginTop: 14, paddingTop: 14, borderTopWidth: 1, borderTopColor: "#0000001A" },
  genBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
