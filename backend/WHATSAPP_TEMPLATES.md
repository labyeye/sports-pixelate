# NestPlay — WhatsApp Message Templates

Checklist for setting up the **new NestPlay WhatsApp Business number**. Every
...
- ✅ = template's body/structure is carried over from the old NestHR number, now recreated under `nestplay_*` names — you can duplicate/export it from the old WABA if Meta allows, otherwise re-create with identical text.
...
- **Sample:** `Priya` · `Pro` · `Ace Sports Academy` · `₹15,000` · `18/07/2027` · `https://app.nestplay.in`
...
`nestplay_*` prefix (previously `neshr_*`, carried over from NestHR) — code
...
"nestplay_...", ...)` call site in `whatsappService.js` together, or sends
will silently fail.
